...
def get_frequencies(mbox, group_by_email):
    ...
    for message in mbox:
        try:
            ...
            full_from = re.findall(r'From: .+', messageString)
            if len(full_from) > 0:
                if group_by_email:
                    matches = re.findall(r'[\w.+-]+@[\w.+-]+', full_from[0])
                    if len(matches) > 0:
                        key = matches[0]
                    else:
                        key = "ScriptError: not found - email"
                        print(key)
                        # print(full_from[0])
                else:
                    key = full_from[0]
            else:
                key = "ScriptError: not found - no from"
                print(key)
                # print(messageString)
            frequencies[key] += 1
        except:
            print("ScriptError: could not parse message")
    return frequencies
